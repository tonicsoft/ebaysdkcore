 ...
 * <br> <B>Input property:</B> <code>ItemDeliveredWithinEDD</code> - This field or the <b>ItemArrivedWithinEDDType</b> field should be included if it is the buyer leaving feedback for the seller. This field will inform eBay about whether or not the buyer received the order line item within the estimated delivery date, which is established once a buyer purchases or commits to buy an item. The value of this field is set to <code>true</code> if the item did arrive within the estimated delivery date, or <code>false</code> if the item arrived past the estimated delivery date.
 * <br>
 * <br>
 * <span class="tablenote"><b>Note:</b> A new "on-time shipping metric" will be a new component of Seller Standards beginning in February of 2016. On-time shipping will mean that the seller shipped the item before the "handling time" expired and/or the item was received by the buyer within the estimated delivery window, which is established once the buyer purchases or commits to buy the item. Previously, a seller's account could be dinged just for getting a low rating for the "shippping time" Detailed Seller Rating. </span>
