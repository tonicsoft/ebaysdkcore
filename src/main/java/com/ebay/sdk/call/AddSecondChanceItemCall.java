 ...
 * <br> <B>Input property:</B> <code>RecipientBidderUserID</code> - This field is used to specify the bidder that is being offered the Second Chance Offer. The eBay User ID of the bidder is used in this field. Specify only one <b>RecipientBidderUserID</b> per call. If multiple users are specified, only the last one specified receives the offer.
 * <br> <B>Input property:</B> <code>BuyItNowPrice</code> - The amount the offer recipient must pay to purchase the item
 * as a Second Chance Offer. This field should only be used when the original
